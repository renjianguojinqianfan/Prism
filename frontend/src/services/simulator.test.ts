...
describe('generateSimReply', () => {
  it('round=1 + 空 history → 返回第一个 deepseek 模板', () => {
    const r = generateSimReply({ simulatorId: 'deepseek' }, '测试话题', [], 1)
  ...
  it('round=2 → 第二个模板（反证法）', () => {
    const r = generateSimReply({ simulatorId: 'deepseek' }, 'X', [], 2)
    expect(r).toContain('反证法')
  })

  it('round=4 wrap 回到第一个模板（(4-1)%3=0）', () => {
    const r1 = generateSimReply({ simulatorId: 'deepseek' }, 'X', [], 1)
    const r4 = generateSimReply({ simulatorId: 'deepseek' }, 'X', [], 4)
    expect(r4).toBe(r1)
  })

  it('simulatorId=kimi → 使用 kimi 模板', () => {
    const r = generateSimReply({ simulatorId: 'kimi' }, '某话题', [], 1)
    expect(r).toContain('换个完全不同的角度')
  })

  it('无 simulatorId → 回退到 deepseek', () => {
    const r = generateSimReply({}, 'X', [], 1)
    expect(r).toContain('逻辑层面')
  })
  ...
  it('history 中存在已完成的 assistant 发言 → 注入 prevNotes', () => {
    ...
    const r = generateSimReply({ simulatorId: 'deepseek' }, 'X', [prev], 1)
  ...
  it('history 中只有 thinking 状态的 assistant → 不应注入 prevNotes', () => {
    ...
    const r = generateSimReply({ simulatorId: 'deepseek' }, 'X', [thinking], 1)
    expect(r).not.toContain('刚才')
  })
  ...
  it('取最后一条已完成的 assistant 发言（reverse().find）', () => {
    ...
    const r = generateSimReply({ simulatorId: 'deepseek' }, 'X', [old, recent], 1)
