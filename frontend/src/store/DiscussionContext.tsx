...
import { PRESET_MODELS, STORAGE_KEY } from '../config/presetModels'
...
export function DiscussionProvider({ children }: { children: ReactNode }) {
  ...
  const dismissToast = (id: string) => dispatch({ type: 'REMOVE_TOAST', id })
  ...
  const setSimulate = (v: boolean) => dispatch({ type: 'SET_SIMULATE', value: v })
  const setMaxRounds = (v: number) => dispatch({ type: 'SET_MAX_ROUNDS', value: v })
  const setInput = (v: string) => dispatch({ type: 'SET_INPUT', value: v })
  const toggleModelEnabled = (id: string) => dispatch({ type: 'TOGGLE_MODEL_ENABLED', id })
  ...
  const removeModel = (idx: number) => dispatch({ type: 'REMOVE_MODEL', idx })
  ...
  const openSettings = () => dispatch({ type: 'SET_SETTINGS_OPEN', value: true })
  const closeSettings = () => dispatch({ type: 'SET_SETTINGS_OPEN', value: false })
  ...
  const generateResponse = async (model: ModelConfig, round: number, simulate: boolean) => {
    ...
    try {
      if (simulate) {
        // 在 push 空消息之前生成回复，避免 prevAi 找到自己刚 push 的空消息
  ...
  const startDiscussion = () => {
    ...
    // 保存 topic 到 ref 供 generateResponse 读取
