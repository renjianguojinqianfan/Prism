...
> **历史文档**：以下内容反映创建时的计划。实际实施时简化了配置：
> - 前端测试从 0 增长至 82 个（非 30+），后端从 5 增长至 63 个（非 40+）
> - vitest 配置未使用 setupFiles 和 coverage，未引入 @testing-library
> - pytest-asyncio 已从 0.23 升级至 1.4.0，respx 已从 0.21 升级至 0.23.1
> - 当前测试配置以 vitest.config.ts 和 pytest.ini 为准
